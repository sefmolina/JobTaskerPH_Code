...
class OrderProcessController extends Controller
{
  public function index(OrderProcessRequest $request, $slug)
  {
    ...
    if ($request['quote_btn_status'] == 1) {
      ...
      } else if ($request['gateway'] == 'paymongo') {
        return $this->handlePaymongo($request, $allData);
      } else {
  ...
  private function handlePaymongo($request, $allData)
  {
    $description = $allData['name'] . ' - ' . $allData['orderNumber'];
    $amount = $allData['grandTotal'];
    ...
    $headers = [
      ...
      "Authorization" => "Basic " . base64_encode("sk_test_UDWEiwgVs38BWR6Lo4K4ezNo:"),
    ...
    if ($response->successful()) {
      ...
      $errorMessage = $response->json()['errors'][0]['detail'] ?? 'Failed to create payment link.';
  ...
  public function cancel($slug, Request $request)
  {
    ...
    return redirect()->route('service.payment_form.check', ['slug' => $slug]);
  }
}
