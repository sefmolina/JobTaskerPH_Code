                        ...
                        <tr>
                          ...
                          <td>
                            <form id="orderStatusForm-{{ $order->id }}" class="d-inline-block"
                              ...
                              @csrf
                              <select
                                class="form-control form-control-sm @if ($order->order_status == 'pending') bg-warning text-dark @elseif ($order->order_status == 'processing') bg-primary @elseif ($order->order_status == 'completed') bg-success @elseif ($order->order_status == 'rejected') bg-danger @endif"
                                name="order_status"
                            ...
                            <td>
                            <form id="orderProgressForm-{{ $order->id }}" class="d-inline-block"
                              action="{{ route('admin.service_order.update_order_progress', ['id' => $order->id]) }}"
                              method="post">
                              @csrf
                              <select
                              class="form-control form-control-sm @if ($order->progress == '10%') bg-warning text-dark @elseif ($order->progress == '20%') bg-primary @elseif ($order->progress == '30%') bg-success @elseif ($order->progress == '40%') bg-danger @elseif ($order->progress == '50%') bg-info @elseif ($order->progress == '60%') bg-secondary @elseif ($order->progress == '70%') bg-dark @elseif ($order->progress == '80%') bg-light @elseif ($order->progress == '90%') bg-muted @elseif ($order->progress == '100%') bg-success @endif"
                              name="progress"
                              onchange="document.getElementById('orderProgressForm-{{ $order->id }}').submit()">
                              <option value="10%" {{ $order->progress == '10%' ? 'selected' : '' }}>
                                10%
                              </option>
                              <option value="20%" {{ $order->progress == '20%' ? 'selected' : '' }}>
                                20%
                              </option>
                              <option value="30%" {{ $order->progress == '30%' ? 'selected' : '' }}>
                                30%
                              </option>
                              <option value="40%" {{ $order->progress == '40%' ? 'selected' : '' }}>
                                40%
                              </option>
                              <option value="50%" {{ $order->progress == '50%' ? 'selected' : '' }}>
                                50%
                              </option>
                              <option value="60%" {{ $order->progress == '60%' ? 'selected' : '' }}>
                                60%
                              </option>
                              <option value="70%" {{ $order->progress == '70%' ? 'selected' : '' }}>
                                70%
                              </option>
                              <option value="80%" {{ $order->progress == '80%' ? 'selected' : '' }}>
                                80%
                              </option>
                              <option value="90%" {{ $order->progress == '90%' ? 'selected' : '' }}>
                                90%
                              </option>
                              <option value="100%" {{ $order->progress == '100%' ? 'selected' : '' }}>
                                100%
                              </option>
                              </select>
                            </form>
