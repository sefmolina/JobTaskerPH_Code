            ...
            <div class="mb-5">
              <div class="btn-group btn-group-toggle d-flex">
                @if ($bs->facebook_login_status == 1)
                  <a class="btn py-2 facebook-login-btn" href="{{ route('seller.login.facebook') }}">
                    <i class="fab fa-facebook-f {{ $currentLanguageInfo->direction == 0 ? 'me-2' : 'ms-2' }}"></i>{{ __('Login via Facebook') }}
                  </a>
                @endif

                @if ($bs->google_login_status == 1)
                  <a class="btn py-2 google-login-btn" href="{{ route('seller.login.google') }}">
          ...
          <div class="user-form" style="background: rgba(255, 255, 255, 0.1); padding: 30px; border-radius: 10px;">
            ...
            <form action="{{ route('seller.login_submit') }}" method="POST">
              ...
              <div class="justify-content-between d-flex mt-3">
                <p class="text-white">{{ __("Don't have an account") . '?' }} <a class="text-white" href="{{ route('seller.signup') }}">{{ __('Signup Now') }}</a></p>
                <a class="text-white" href="{{ route('seller.forget.password') }}">{{ __('Lost your password?') }}</a>
              </div>
            </form>
          </div>
        </div>
